...
function calc() {
  ...
  calcTotal();
  function getStaticInforamtion(selector, activeSelector) {
    const elements = document.querySelectorAll(selector);
    elements.forEach(elem => {
...
function cards() {
  ...
  const urlDb = 'http://localhost:3000/menu';
  (0,_services_sevices__WEBPACK_IMPORTED_MODULE_0__.getResource)(urlDb).then(data => {
    data.forEach(({
      img,
      altimg,
      title,
...
function forms(formSelector, modalTimerId) {
  ...
  function bindPostData(form) {
    form.addEventListener('submit', e => {
      ...
      const requestDb = 'http://localhost:3000/requests';
      (0,_services_sevices__WEBPACK_IMPORTED_MODULE_1__.postData)(requestDb, json).then(data => {
        console.log(data);
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  (0,_modules_timer__WEBPACK_IMPORTED_MODULE_2__["default"])('.timer', '2024-08-24');
